...
class DCEL:
    ...
    def findHorizens(self,p):
        # horizons = []
        # if len(p.conflict_facets) == 1:
        #     print(1)
        #     f = p.conflict_facets[0]
        ...
        horizon_edges = []
        cpoints = set()
        for facet in p.conflict_facets:
            horizon_edges.extend(facet.get_edges())

        for i, facet1 in enumerate(p.conflict_facets):
            for j, facet2 in enumerate(p.conflict_facets):
                if i != j:
                    for x in facet1.conflict_points:
                        cpoints.add(x)
                    for x in facet2.conflict_points:
                        cpoints.add(x)
                    ...
                    if edge_1 != None:
                        ...
                        try:
                            ...
                            pass
        # print("edges",horizon_edges)
        # print("points",cpoints)
        return horizon_edges,cpoints

    def create_new_facets(self,p,h):
        new=[]
        for edge in h:
            # print(edge)
            self.facets.append(Facet(p,edge[0],edge[1]))
            new.append(Facet(p,edge[0],edge[1]))
        return new



    def remove_facets(self,p):
        for facet in p.conflict_facets:
            try:
    ...
    def update_conflict_points(self,cp,new,p):
        for i in cp:
            for o in p.conflict_facets:
                try:
                    i.conflict_facets.remove(o)
                except:
                    pass
            for n in new:
                if orient(n.p1, n.p2, n.p3, i) == -1:
                    i.conflict_facets.append(n)
...
def convex_hull(points):
    ...
    for i in range(4,len(points)):
        if len(points[i].conflict_facets) == 0:
           ...
           h,cp = dcel.findHorizens(points[i])
           dcel.remove_facets(points[i])
           new = dcel.create_new_facets(points[i],h)
           dcel.update_conflict_points(cp,new,points[i])
    print(dcel)
